...
class OfflineBufferH5():
    def __init__(self, datapaths, offline_data_size=2000, device="cpu", random_sample=True):
        ...
        if 'actions_onehot' not in self.keys and 'actions' in self.keys:
            num_actions = self.data['avail_actions'].shape[-1]
            self.data['actions_onehot'] = np.eye(num_actions, dtype=np.float32)[self.data['actions'].astype(dtype=np.int32)] 
            
        self.data['actions'] = self.data['actions'].astype(dtype=int)
        self.data['actions_onehot'] = self.data['actions_onehot'].astype(dtype=int)
        self.data['avail_actions'] = self.data['avail_actions'].astype(dtype=int)
        self.data['filled'] = self.data['filled'].astype(dtype=int)
        self.data['obs'] = self.data['obs'].astype(dtype=np.float32)
        self.data['reward'] = self.data['reward'].astype(dtype=np.float32)
        self.data['state'] = self.data['state'].astype(dtype=np.float32)
        self.data['terminated']= self.data['terminated'].astype(dtype=int)
