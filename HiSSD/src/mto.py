...
def init_tasks(task_list, main_args, logger):
    ...
    for task in task_list:
        ...
        if task_args.env == "sc2" or task_args.env == "smacv2":
        ...
        if main_args.env == "sc2" or main_args.env == "smacv2":
...
def train_sequential(
    ...
    while t_env < t_max:
        ...
        for task in train_tasks:
            ...
            # t_env += 1
            t_env += episode_sample['filled'].sum().item()
            ...
            with th.no_grad():
                ...
                if pretrain and test_task2offlinedata is not None:
                    for task, data_buffer in test_task2offlinedata.items():
                        ...
                        # episode_sample = data_buffer.sample(batch_size_train * 10) # NOTE: why was this 10?
                        episode_sample = data_buffer.sample(batch_size_train * 3)
...
def run_sequential(args, logger):
    ...
    main_args.offline_data_folder = os.path.join(
        main_args.offline_data_folder, 
        f"dataset-{args.env_args['map_name']}",
        # "expert", 
    )
