...
from .arrays import apply_dict, batch_to_device, to_device, to_np, batchify

from .timer import Timer


def graph_collate_fn(data):
    """ Custom collate function to handle batching the DGL graphs which are stored in arrays of type object

    Args:
        data (List): List of length batch_size

    Returns:
        Dict: Batch dictionary with the same keys as the input data but with the values batched 
    """
    
    batch_ready_data = [batchify(d, 'cpu') for d in data]
    batched_data = {}
    for key in batch_ready_data[0].keys():                
        if torch.is_tensor(batch_ready_data[0][key]):
            batched_data[key] = torch.cat([d[key] for d in batch_ready_data], dim=0)
                
        elif type(batch_ready_data[0][key]) is dict:
            batched_data[key] = {k: torch.cat([d[key][k] for d in batch_ready_data], dim=0) 
                                 for k in batch_ready_data[0][key].keys()}
            
        elif isinstance(batch_ready_data[0][key], type(np.array([]))):
            batched_data[key] = np.concatenate([d[key] for d in batch_ready_data], axis=0)
...
class Trainer(object):
    def __init__(
        ...
        assert (eval_freq % save_freq == 0), f"eval_freq must be a multiple of save_freq, but got {eval_freq} and {save_freq} respectively"
        ...
        self.use_hetgat_attention = use_hetgat_attention
        ...
        if dataset is not None:
            ...
            self.dataloader = cycle(torch.utils.data.DataLoader(self.dataset, 
                                                                ...
                                                                shuffle=True, 
                                                                pin_memory=True, 
                                                                collate_fn=graph_collate_fn if self.use_hetgat_attention else None,
                                                                )
                                    )

            self.dataloader_vis = cycle(torch.utils.data.DataLoader(self.dataset, 
                                                                    batch_size=1, 
                                                                    num_workers=0, 
                                                                    shuffle=True, 
                                                                    pin_memory=True,
        ...
        if self.save_training_metrics: 
            # load the training metrics if they exist 
            save_file_path = os.path.join(self.bucket, logger.prefix, "metrics.json")
            if os.path.exists(save_file_path):
                self.training_metrics = logger.load_json(save_file_path)
    ...
    def train(self, n_train_steps):
        ...
        for _ in range(n_train_steps):
            ...
            if self.step % self.log_freq == 0:
                ...
                logger.log(step=self.step, loss=loss.detach().item(), **metrics, flush=True)

                # log to tensorboard
                if self.tb_logger is not None:
                    self.tb_logger.add_scalar("loss", loss.detach().item(), self.step)
                    
                    for key, val in infos.items():
                        self.tb_logger.add_scalar(key, val.detach().item(), self.step)
                        
                if self.save_training_metrics:         
                    
                    save_dict = {}
                    save_dict["loss"] = loss.detach().item()
                    ...
                    self.training_metrics[self.step] = save_dict
                    
                    save_file_path = os.path.join(self.bucket, logger.prefix, "metrics.json")
                    logger.save_json(self.training_metrics, save_file_path)
    ...
    def evaluate(self):
        assert (self.evaluator is not None), "Method `evaluate` can not be called when `self.evaluator` is None. Set evaluator with `self.set_evaluator` first."
    ...
    def render_samples(self, batch_size=2, n_samples=2):
        ...
        for i in range(batch_size):
            ...
            if self.ema_model.returns_condition:
                returns = to_device(torch.ones(n_samples, 1, self.model.n_agents), self.device)
    ...
    def inv_render_samples(self, batch_size=2, n_samples=2):
        ...
        for i in range(batch_size):
            ...
            # repeat each item in conditions `n_samples` times
            conditions = apply_dict(einops.repeat, conditions, "b ... -> (repeat b) ...", repeat=n_samples,)
