...
def to_device(x, device=DEVICE):
    ...
    elif isinstance(x, type(np.array([]))):
        return x
...
def batchify(batch, device):
    ...
    for field in batch.keys():
        val = batch[field]
        if field == "graphs": # shape (b, t, a, f)
            val = apply_graphs(val[None], device)
...
def apply_graphs(x, device):
    """" 
    Move all DGL graphs to CUDA. Data cannot be stored as tensor since its of type object
    
    """
    device = device or DEVICE
    batch_shape = x.shape
    
    return np.array([graph.to(device) for graph in x.flatten()], dtype=object).reshape(batch_shape)



def remove_player_info(x, condition):
    if ("player_idxs" in condition and "player_hoop_sides" in condition and x.shape[-1] == 4):  # must have add player info
        x = x[:, :, :, 1:-1]
    return x
...
def batch_to_device(batch, device="cuda:0"):
    ...
    for k in batch.keys():
        ...
        if k == "graphs":
            device_batch[k] = apply_graphs(batch[k], device)
        else:
            device_batch[k] = to_device(batch[k], device)
