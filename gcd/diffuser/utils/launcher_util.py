...
# RUN.script_root = os.path.abspath(os.path.dirname(os.path.dirname(os.path.dirname(__file__))))

RUN.script_root = os.path.join(os.environ.get('LOG_DIR_PREFIX'))
...
def set_config_override_args(Config, override_args):
    
    args_dict = {}
    for i in range(0, len(override_args), 2):
        key = override_args[i][2:]
        value_str = override_args[i + 1]
        
        # Convert the value string to appropriate type
...
def update_exp_spec_dict(exp_specs, ovveride_args_dict):     
    for k, v in ovveride_args_dict.items():
        exp_specs[k] = v
