...
from diffuser.datasets.smacv2_dist_configs import return_distribution_config
...
class SMACv2(gym.Env):
    ...
    def __init__(self, scenario, add_agent_ids_to_obs: bool = True, subteam: Optional[str] = '', prob_obs_enemy=1.0):
...
def sequence_dataset(env, preprocess_fn, blind_own_type = False):
    ...
    # (ablation) blind agent's own type to see the effectiveness of diverse gnn modules
    if blind_own_type:
        # a = observations[-50:, :, :]
        observations[:, :, -3:] = -1
    ...
    for path_length in path_lengths:
        ...
        if env.use_hetgat_attention:
            ntypes, graphs, etypes = get_graph_info(episode_states[:, 0, :], env.num_agents, info['state_features'])
