...
class SequenceDataset(torch.utils.data.Dataset):
    def __init__(self, 
        ...
        self.env = env = env_mod.load_environment(env, debugging, use_hetgat_attention)
        ...
        elif env_type == "smac" or env_type == "smac_v2":
            itr = env_mod.sequence_dataset(env, self.preprocess_fn, blind_own_type=blind_own_type)

        else:
            itr = env_mod.sequence_dataset(env, self.preprocess_fn)

        fields = ReplayBuffer(n_agents, 
                              max_n_episodes, 
                              max_path_length, 
                              termination_penalty, 
    ...
    def pad_future(self, keys: List[str] = None):
        if keys is None:
            ...
            if self.use_hetgat_attention:
                keys += [ "ntypes", "graphs", "etypes"]
        ...
        for key in keys:
            ...
            if self.use_zero_padding:
                ...
                self.fields[key] = np.concatenate([self.fields[key], 
    ...
    def pad_history(self, keys: List[str] = None):
        ...
        for key in keys:
            shape = self.fields[key].shape
            if self.use_zero_padding:
                self.fields[key] = np.concatenate([np.zeros((shape[0], self.history_horizon, *shape[2:]), 
                                                            dtype=self.fields[key].dtype,),
                                                   self.fields[key],], 
                                                  axis=1,)
            else:
                self.fields[key] = np.concatenate([np.repeat(self.fields[key][:, :1], self.history_horizon, axis=1,),
                                                   self.fields[key],],
                                                  axis=1,)
    ...
    def __getitem__(self, idx: int, agent_idx: Optional[int] = None):
        ...
        if self.agent_condition_type == "single": 
            path_ind, start, end, mask_end = self.indices[idx // self.n_agents] # shape: (num_data_point, *(i, start, end, mask_end)) 
        ...
        observations = self.fields.normed_observations[path_ind, history_start:end] #(num_agents, )
        
        if self.use_hetgat_attention: 
            ntypes = self.fields.ntypes[path_ind, history_start:end]
            graphs = self.fields.graphs[path_ind, history_start:end]
            etypes = self.fields.etypes[path_ind, history_start:end]
        ...
        if self.include_returns:
            ...
            returns = np.array([returns / self.returns_scale], dtype=np.float32) # scaled sum of discounted returns 
