...
class GaussianDiffusion(nn.Module):
    def __init__(self,
                 ...
                 # Remove the unused **kwargs parameter
                 ):
        
        
        assert action_dim > 0
        assert ( not returns_condition or not returns_loss_guided), "Can't do both returns conditioning and returns loss guidence"
        ...
        # TODO: handle varying observation dimension inputs 
        self.observation_dim = observation_dim
        
        # action dim shared across agents (need to double check for other SMAC teams)
        ...
        self.het_inv = het_inv
        
        self.data_encoder = data_encoder

        if self.use_inv_dyn:
            self.inv_model = self._build_inv_model(hidden_dim, 
        ...
        self.noise_scheduler = DDPMScheduler(num_train_timesteps=self.n_timesteps,
                                             clip_sample=True,
    ...
    # TODO: modify model to handle different number of agents/observation dimensions
    # if shared inv, then only need to handle observation_dim 
    
    def _build_inv_model(self, hidden_dim: int, output_dim: int, num_classes: int = 3):
        ...
        if self.joint_inv:
            ...
            in_dims = [2*self.observation_dim, hidden_dim, hidden_dim]
            out_dims = [hidden_dim, hidden_dim, output_dim]

            for layer_idx in range(len(in_dims)):
                typed_linear_layer = TypedLinear(in_dims[layer_idx], out_dims[layer_idx], num_classes)
                inv_model.append(typed_linear_layer)
                
            self.inv_model_activation = nn.ReLU()
    ...
    def get_model_output(self, 
                         ...
                         ntypes: Optional[torch.Tensor] = None, 
                         graphs: Optional[np.array] = None, 
                         etypes: Optional[torch.Tensor] = None,
                         decentralized_execution: bool = False, 
                         ):
        
        # Access the "states" variable
        # print(states)
        # Rest of the code...
    ...
    def conditional_sample(self, 
        ...
        # TODO: code dependent on the number of agents/observation dim of the task
        ...
        for t in timesteps:
            ...
            model_output = self.get_model_output(x, 
                                                 ts, 
                                                 returns, 
                                                 env_ts, 
                                                 attention_masks, 
    ...
    def p_losses(self,
        ...
        if "agent_idx" in cond.keys() and self.opponent_loss_weight is not None:
            ...
            indices = (cond["agent_idx"].to(torch.long)[..., None].repeat(1, opponent_loss_weight.shape[1], 1, opponent_loss_weight.shape[-1]))
    ...
    def r_losses(self, x_t, t, noise, cond):
        ...
        model_mean, _, model_log_variance = self.q_posterior(x_start=x_recon, x_t=x_t, t=t)
    ...
    def compute_inv_loss(self, 
        ...
        x_comb_t = rearrange(x_comb_t, "b t a f -> (b t) a f")
        ...
        if self.joint_inv or self.share_inv or self.het_inv:
            
            if self.joint_inv: # (b a) f
                pred_a_t = self.inv_model(x_comb_t.reshape(x_comb_t.shape[0], -1)).reshape(x_comb_t.shape[0], 
                                                                                           x_comb_t.shape[1], 
                                                                                           -1)
                ...
                ntypes_t = ntypes[:, :-1]
                ntypes_t = rearrange(ntypes_t, "b t a -> (b t a)")
                out = rearrange(x_comb_t, "(bt) a f -> (bt a) f")
                
                for layer_idx, layer in enumerate(self.inv_model): 
                    out = layer(out, ntypes_t)            
                    if layer_idx < len(self.inv_model) - 1: # pass through activation all but last layer 
                        out = self.inv_model_activation(out)
                
                pred_a_t = rearrange(out, "(bt a) f -> bt a f", bt=legal_actions_t.shape[0], a=legal_actions_t.shape[1])
            ...
            if self.discrete_action:
                inv_loss = (F.cross_entropy(pred_a_t.reshape(-1, pred_a_t.shape[-1]), a_t.reshape(-1).long(), reduction="none",)* masks_t.reshape(-1)).mean() / masks_t.mean()
                inv_acc = ((pred_a_t.argmax(dim=-1, keepdim=True) == a_t).to(dtype=float).squeeze(-1)* masks_t).mean() / masks_t.mean()
                info["inv_acc"] = inv_acc
                
            else:
                inv_loss = (F.mse_loss(pred_a_t, a_t, reduction="none") * masks_t.unsqueeze(-1)).mean() / masks_t.mean()


        else: # independent inv
            
            inv_loss = 0.0            
            for i in range(self.n_agents):
                pred_a_t = self.inv_model[i](x_comb_t[:, i])
                if self.discrete_action:
                    inv_loss += (F.cross_entropy(pred_a_t, a_t[:, i].reshape(-1).long(), reduction="none")* masks_t[:, i]).mean() / masks_t[:, i].mean()
                else:
                    inv_loss += (F.mse_loss(pred_a_t, a_t[:, i]) * masks_t[:, i].unsqueeze(-1)).mean() / masks_t[:, i].mean()
    ...
    def loss(self, 
        ...
        if self.train_only_inv:
            ...
            batch_size = len(x)
            t = torch.randint(0, self.noise_scheduler.config.num_train_timesteps, (batch_size,), device=x.device,).long()
        ...
        if self.use_inv_dyn:
            ...
            if self.het_inv:
                inv_loss, inv_info = self.compute_inv_loss(x, loss_masks, legal_actions, ntypes)
...
class GaussianDiffusionBCMod(nn.Module):
    def __init__(self,
                 model,
                 n_agents: int,
                 horizon: int,
    ...
    def get_model_output(self, 
        ...
        x_input = x[:,:self.history_horizon, :, :]

        epsilon = self.model(x_input, 
                             t, 
                             returns=returns, 
                             env_timestep=env_ts, 
                             attention_masks=attention_masks,
    ...
    def conditional_sample(self, 
        ...
        ts = torch.full((batch_size,), 0, device=device, dtype=torch.long) # this info is not actually used 
        
        x = apply_conditioning(x, cond) # to get the conditioning info; in get_model_output, we only use history_horizon 
        
        x = self.get_model_output(x, 
                                  ts, 
                                  returns, 
                                  env_ts, 
                                  attention_masks, 
    ...
    def p_losses(self,
        ...
        x_input = x_start[:,:self.history_horizon, :, :]    
        ...
        # TODO: what does this do ? do we need it cause we will set self.predic_epsilon false (want to predict the actual states)
        ...
        # no longer true since epsilon shape is history_horizon and noise is history_horizon + prediction horizon
        # assert noise.shape == epsilon.shape
