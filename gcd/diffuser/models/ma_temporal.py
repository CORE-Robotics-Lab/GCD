...
class ConvAttentionDeconv(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        assert (x.shape[2] == self.n_agents), f"Expected {self.n_agents} agents, but got samples with shape {x.shape}"
        ...
        if self.returns_condition:
            ...
            returns_embed = [self.nets[i].returns_mlp(returns[:, :, i]) for i in range(self.n_agents)]
            
            
            if use_dropout:
                # here use the same mask for all agents
                mask = (self.nets[0].mask_dist.sample(sample_shape=(returns_embed[0].size(0), 1)).to(returns_embed[0].device))
...
class SharedConvAttentionDeconv(nn.Module):
    ...
    def __init__(self, 
        ...
        # TODO: dependence on the number of agents    
        self.n_agents = n_agents
        self.history_horizon = history_horizon
        self.use_temporal_attention = use_temporal_attention
        ...
        # TODO: transition_dim specifies observation dimensions
        dims = [transition_dim, *map(lambda m: dim * m, dim_mults)]
        in_out = list(zip(dims[:-1], dims[1:]))
        print(f"[ models/temporal ] Channel dimensions: {in_out}")

        self.net = TemporalUnet(horizon=horizon,
                                history_horizon=history_horizon,
                                transition_dim=transition_dim,
                                dim=dim,
                                dim_mults=dim_mults,
        ...
        # def __init__(
        #     self,
        #     n_channels: int, (1024) (input)
        #     qk_n_channels: int, (256) 
        #     v_n_channels: int, (1024) (output)
        #     embed_dim: int,
        #     nheads: int = 4,
        #     residual: bool = False,
        # ):
        
        self.self_attn = [TemporalSelfAttention(in_out[-1][1], 
                                                in_out[-1][1] // 16, 
                                                in_out[-1][1] // 4, 
                                                residual=residual_attn, 
                                                embed_dim=self.net.embed_dim)]
        for dims in reversed(in_out):
            self.self_attn.append(TemporalSelfAttention(dims[1],
                                                        dims[1] // 16,
                                                        dims[1] // 4,
                                                        residual=residual_attn,
                                                        embed_dim=self.net.embed_dim,))
    ...
    def forward(self,
        ...
        if self.returns_condition:
            ...
            if use_dropout:
                # here use the same mask for all agents
                mask = self.net.mask_dist.sample(sample_shape=(returns_embed.size(0), returns_embed.size(1), 1)).to(returns_embed.device)
        ...
        if self.env_ts_condition: # not used in experiment configs
            assert env_timestep is not None
        ...
        h = []
        x = x.reshape(x.shape[0] * x.shape[1], x.shape[2], x.shape[3]) # (b, a, f, t) -> (b * a, f, t)
        t = t.reshape(t.shape[0] * t.shape[1], t.shape[2]) # (b, a, f) -> (b * a, f)
        ...
        x = x.reshape(bs, x.shape[0] // bs, x.shape[1], x.shape[2]) # (b, a, f, t), why is t=6? 
        ...
        for layer_idx in range(len(self.net.ups)):
            hiddens = h.pop()
            hiddens = hiddens.reshape(bs, hiddens.shape[0] // bs, hiddens.shape[1], hiddens.shape[2])
            ...
            hiddens = hiddens.reshape(hiddens.shape[0] * hiddens.shape[1], hiddens.shape[2], hiddens.shape[3])
...
class SharedConvHetGATDeconv(nn.Module):
    agent_share_parameters = True
    ...
    def __init__(self, 
                 ...
                 max_path_length: int = 100,
                 use_temporal_attention: bool = True,
                 num_classes: int = 3, # SMAC scenarios have 3 classes maximum 
                 ):
        super().__init__()
                
        self.n_agents = n_agents
        self.num_classes = num_classes
        self.num_relations = num_classes**2 # (class_i, class_j)
        self.history_horizon = history_horizon
        self.use_temporal_attention = use_temporal_attention
        ...
        self.self_attn = [TemporalHetGAT(in_feat=in_out[-1][1], 
                                         out_feat=in_out[-1][1] // nhead, 
        ...
        for dims in reversed(in_out): 
            self.self_attn.append(TemporalHetGAT(in_feat=dims[1], 
                                                 out_feat=dims[1] // nhead, 
                                                 num_relations=self.num_relations,
                                                 num_classes=self.num_classes,
                                                 embed_dim=self.net.embed_dim,
                                                 num_heads=nhead, 
    ...
    def forward(self,
        ...
        if self.returns_condition:
            ...
            if use_dropout:
                # here use the same mask for all agents
                mask = self.net.mask_dist.sample(sample_shape=(returns_embed.size(0),
                                                               returns_embed.size(1), 1)
                                                 ).to(returns_embed.device)
                returns_embed = mask * returns_embed   # TODO: this is only dropping out 1 agents? 
        ...
        # TODO: not used in experiment configs
        if self.env_ts_condition:
            assert env_timestep is not None
            env_timestep = env_timestep.to(dtype=torch.int64)
            env_timestep = env_timestep[:, self.history_horizon]
        ...
        if self.graph_condition and use_dropout: 
            hetgat_mask = self.net.mask_dist.sample(sample_shape=(returns_embed.size(0), 
                                                                  returns_embed.size(1), 
                                                                  1)
                                                    ).to(returns_embed.device)
        ...
        if self.use_temporal_attention:
            ...
            # compute graph attention given graph structure at time t (last time step in the history horizon)
            x = self.self_attn[0](x, 
                                  t,
                                  ntypes=ntypes[:, self.history_horizon, :], 
                                  graphs=graphs[:, self.history_horizon, :], 
                                  etypes=etypes[:, self.history_horizon, :], 
                                  use_dropout=use_dropout, 
                                  force_dropout=force_dropout, 
                                  mask=hetgat_mask if (self.graph_condition and use_dropout) else None, 
                                  )
        ...
        for layer_idx in range(len(self.net.ups)):
            ...
            if self.use_temporal_attention: # compute graph attention given graph structure at time t (last time step in the history horizon)
                t = t.reshape(bs, t.shape[0] // bs, t.shape[1])    
                hiddens = self.self_attn[layer_idx + 1](hiddens, 
                                                        t, 
                                                        ntypes=ntypes[:, self.history_horizon, :], 
...
class SharedConvHetGATDeconvFixed(nn.Module):
    agent_share_parameters = True
    ...
    def __init__(self, 
        ...
        self.net = TemporalUnetFixed(horizon=horizon, 
                                     history_horizon=history_horizon,
                                     transition_dim=transition_dim,
                                     dim=dim,
                                     dim_mults=dim_mults,
        ...
        self.hetgat_layers = []
        for dims in reversed(in_out): 
            self.hetgat_layers.append(TemporalHetGAT(in_feat=dims[1], 
                                                     out_feat=dims[1] // nhead, 
                                                     num_relations=self.num_relations, num_classes=self.num_classes, 
                                                     embed_dim=self.net.embed_dim,
                                                     num_heads=nhead, 
                                                     residual=residual_attn, 
                                                     graph_condition=graph_condition,
        ...
        self.hetgat_layers = nn.ModuleList(self.hetgat_layers)
    ...
    def forward(self,
        ...
        if self.returns_condition:
            ...
            if use_dropout: # here use the same mask for all agents
                mask = self.net.mask_dist.sample(sample_shape=(returns_embed.size(0),
                                                               returns_embed.size(1), 1)
                                                 ).to(returns_embed.device)
                returns_embed = mask * returns_embed  
        ...
        if self.env_ts_condition: # NOTE: not used in experiment configs
            assert env_timestep is not None
            env_timestep = env_timestep.to(dtype=torch.int64)
            env_timestep = env_timestep[:, self.history_horizon]
            env_ts_embed = self.net.env_ts_mlp(env_timestep)
        ...
        x = x.reshape(bs, x.shape[0] // bs, x.shape[1], x.shape[2]) # (b, a, f, t)
        ...
        for layer_idx in range(len(self.net.ups)):
            ...
            if self.use_temporal_attention: # compute graph attention given graph structure at time t (last time step in the history horizon)
                t = t.reshape(bs, t.shape[0] // bs, t.shape[1])    
                hetgat_embedding = self.hetgat_layers[layer_idx](hiddens, 
                                                                 t, 
                                                                 ntypes=ntypes[:, self.history_horizon, :], 
                                                                 graphs=graphs[:, self.history_horizon, :], 
                                                                 etypes=etypes[:, self.history_horizon, :], 
                                                                 ...
                                                                 force_dropout=force_dropout, 
                                                                 mask=hetgat_mask if (self.graph_condition and use_dropout) else None, 
                                                                 decentralized_execution=decentralized_execution
                                                                 )
            ...
            hetgat_embedding = hetgat_embedding.reshape(hetgat_embedding.shape[0] * hetgat_embedding.shape[1], hetgat_embedding.shape[2], hetgat_embedding.shape[3])
            resnet, resnet2, upsample = self.net.ups[layer_idx]
            x = torch.cat((x, hetgat_embedding), dim=1)
...
class SharedConvHetGCNDeconvFixed(nn.Module):
    agent_share_parameters = True
    ...
    def __init__(self, 
        ...
        self.hetgcn_layers = []
        for dims in reversed(in_out): 
            self.hetgcn_layers.append(TemporalHetGCN(in_feat=dims[1], 
                                                     out_feat=dims[1], 
                                                     num_relations=self.num_relations, num_classes=self.num_classes, 
                                                     embed_dim=self.net.embed_dim,
                                                     residual=residual_attn, 
        ...
        self.hetgcn_layers = nn.ModuleList(self.hetgcn_layers)
    ...
    def forward(self,
        ...
        for layer_idx in range(len(self.net.ups)):
            ...
            if self.use_temporal_attention: # compute graph attention given graph structure at time t (last time step in the history horizon)
                t = t.reshape(bs, t.shape[0] // bs, t.shape[1])    
                hetgat_embedding = self.hetgcn_layers[layer_idx](hiddens, 
                                                                 t, 
                                                                 ntypes=ntypes[:, self.history_horizon, :], 
                                                                 graphs=graphs[:, self.history_horizon, :], 
                                                                 etypes=etypes[:, self.history_horizon, :], 
...
class SharedConvGCNDeconvFixed(nn.Module):
    agent_share_parameters = True
    ...
    def __init__(self, 
        ...
        self.gcn_layers = []
        for dims in reversed(in_out): 
            self.gcn_layers.append(TemporalGCN(in_feat=dims[1], 
                                               out_feat=dims[1], 
                                               num_relations=self.num_relations, num_classes=self.num_classes, 
                                               embed_dim=self.net.embed_dim,
                                               residual=residual_attn, 
    ...
    def forward(self,
        ...
        for layer_idx in range(len(self.net.ups)):
            ...
            if self.use_temporal_attention: # compute graph attention given graph structure at time t (last time step in the history horizon)
                t = t.reshape(bs, t.shape[0] // bs, t.shape[1])    
                hetgat_embedding = self.gcn_layers[layer_idx](hiddens, 
                                                              t, 
                                                              ntypes=ntypes[:, self.history_horizon, :], 
                                                              graphs=graphs[:, self.history_horizon, :], 
                                                              etypes=etypes[:, self.history_horizon, :], 
...
class SharedConvHetGATDeconvFixedBCMod(nn.Module):
    agent_share_parameters = True
    ...
    def __init__(self, 
        ...
        self.net.returns_mlp[4] = nn.Linear(512, 256, bias=True)
       
        # replaces last identity upconv block such that the last upconv layer make hidden dim goes from 20 (input horizon dim to unet), to 24 (output horizon dim)
        
        self.net.ups[2][2] = nn.ConvTranspose1d(128, 128, kernel_size=5, stride=1, padding=0)
    ...
    def forward(self,
        ...
        # removing this embedding since we are not diffusing 
        # t = self.net.time_mlp(torch.stack([time for _ in range(x.shape[1])], dim=1))
        ...
        if self.returns_condition:
            ...
            # remove the "time" here since we are not diffusing
            t = torch.cat([returns_embed], dim=-1)
...
class SharedConvHetGATDeconvV2(nn.Module):
    agent_share_parameters = True
    ...
    def __init__(self, 
        ...
        self.self_attn = TemporalHetGATV2(in_feat=in_out[-1][1], 
                                          out_feat=in_out[-1][1] // nhead, 
                                          num_relations=self.num_relations,
                                          num_classes=self.num_classes,
                                          embed_dim=self.net.embed_dim,
    ...
    def forward(self,
        ...
        if self.use_temporal_attention:
            ...
            # compute graph attention given the graph info at time t (last time step in the history horizon)            
            x = self.self_attn(x, 
                               t, 
                               ntypes=ntypes[:, self.history_horizon, :], 
                               graphs=graphs[:, self.history_horizon, :], 
...
class SharedAttentionAutoEncoder(nn.Module):
    ...
    def __init__(
        ...
        self.final_mlp = nn.Sequential(nn.Linear(dim, dim), act_fn, nn.Linear(dim, transition_dim))
