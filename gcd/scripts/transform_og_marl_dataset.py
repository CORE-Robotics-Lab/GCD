...
from og_marl.environments import smac, smacv2, mamujoco
...
def main(env_name: str, map_name: str, quality: str):
    ...
    dataset_dir = Path(f"{os.environ.get('DATASET_DIR_PREFIX')}/{env_name}/{map_name}/{quality}")
    ...
    def get_fname_idx(file_name):
        if env_name == "smac" or env_name == "smac_v2":
    ...
    raw_dataset = filename_dataset.flat_map(
        lambda x: tf.data.TFRecordDataset(x, compression_type="GZIP").map(env._decode_fn))
    ...
    (all_observations, all_actions, all_rewards, all_discounts, all_logprobs,) = ([], [], [], [], [])
    ...
    path_length = 0
    (path_observations, path_actions, path_rewards, path_logprobs, path_discounts,) = ([], [], [], [], [])
    ...
    for databatch in tqdm(databatches):
        ...
        for idx in range(batch_size):
            ...
            if (int(path_discounts[-1][-1, 0]) == 0 or path_length >= env.max_episode_length):
                ...
                (path_observations, path_actions, path_rewards, path_discounts, path_logprobs,) = ([], [], [], [], [])
    ...
    """ Save Numpy Arrays """
    if env_name == "smac" or env_name == "smac_v2":
        np.save(f"{os.environ.get('DATASET_DIR_PREFIX')}/{env_name}/{map_name}/{quality}/states.npy", all_states,)
        np.save(f"{os.environ.get('DATASET_DIR_PREFIX')}/{env_name}/{map_name}/{quality}/legals.npy", all_legals,)
    
    np.save(f"{os.environ.get('DATASET_DIR_PREFIX')}/{env_name}/{map_name}/{quality}/obs.npy", all_observations,)
    np.save(f"{os.environ.get('DATASET_DIR_PREFIX')}/{env_name}/{map_name}/{quality}/actions.npy", all_actions,)
    np.save(f"{os.environ.get('DATASET_DIR_PREFIX')}/{env_name}/{map_name}/{quality}/rewards.npy", all_rewards,)
    np.save(f"{os.environ.get('DATASET_DIR_PREFIX')}/{env_name}/{map_name}/{quality}/discounts.npy", all_discounts,)
    
    if "logprobs" in extras:
        np.save(f"{os.environ.get('DATASET_DIR_PREFIX')}/{env_name}/{map_name}/{quality}/logprobs.npy", all_logprobs,)
        
    np.save(f"{os.environ.get('DATASET_DIR_PREFIX')}/{env_name}/{map_name}/{quality}/path_lengths.npy", all_path_lengths,)
