...
def collect_sequential(args, runner):
    ...
    while collected_episodes<args.collect_nepisode:
        if args.h5file_suffix=='random':
            ...
            runner.t_env=0
        elif args.h5file_suffix=='pre_defined':
            # batch_tmp = runner.run(pre_defined_mode=True)
            batch_tmp = runner.collect_predefined(test_mode=True)
        else:
...
def run_sequential(args, logger):
    ...
    if args.use_offline:
        ...
        for key, value in total_datas.items():
            if isinstance(value, th.Tensor):
                all_zero = (value == 0).all().item()
                print(f"{key}: all zero? {all_zero} | shape={tuple(value.shape)}")
            else:
                print(f"{key}: not a torch.Tensor (type={type(value)})")
    ...
    while runner.t_env <= args.t_max:
        ...
        if args.save_model and (runner.t_env - model_save_time >= save_model_interval or model_save_time == 0):
            ...
            if 'map_name' in args.env_args.keys():
                save_path=os.path.join(dirname(dirname(dirname(abspath(__file__)))), args.local_results_path)

                save_path = os.path.join(save_path, "models", args.env_args['map_name'],args.unique_token, str(runner.t_env))
            else:
                raise ValueError("`map_name` is missing from args.env_args")
