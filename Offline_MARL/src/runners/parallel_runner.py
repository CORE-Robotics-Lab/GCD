...
import numpy as np
import torch as th
from share_array import get_shared_array
...
class ParallelRunner:
    ...
    def __init__(self, args, logger):
        ...
        if "ocean" in args.env:
            shared_wave_info = get_shared_array("wave_info")
            for conn in self.parent_conns:
                conn.send(("set_wave_info", shared_wave_info))
    ...
    def run(self, test_mode=False):
        ...
        if test_mode and (len(self.test_returns) == n_test_runs):
            ...
            self._log(cur_returns, cur_stats, "pre_test")
    ...
    def collect_predefined(self, test_mode=False):
        self.reset()
...
def env_worker(remote, env_fn):
    ...
    while True:
        ...
        elif cmd == "set_wave_info":
            env.whole_intensity_data = data
        elif cmd == "reset":
            env.reset()
            remote.send({
                "state": env.get_state(),
